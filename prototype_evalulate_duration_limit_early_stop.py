...
class PrototypeSingleton:
    ...
    PARAM_GRIDS = {
        ...
        "discretize":{
            KBinsDiscretizer:{"n_bins":[3, 5, 10, 20],"encode": ['onehot', 'onehot-dense', 'ordinal'],"strategy": ['uniform', 'quantile', 'kmeans']},
...
class BayesianPipelineOptimizer:
    def __init__(self, steps_order, model, n_trials=50, cv=3, trial_timeout=None):
        """
        trial_timeout: 每个 trial 的最大允许执行时间（秒）。若为 None 则不限制（默认）。
        """
    ...
    def _build_dynamic_pipeline(self, trial, X):
        ...
        if self.model_type in self.model_config:
            model_instance = self.model_config[self.model_type]
        else:
            # fallback
            first_model_key = next(iter(self.model_config))
            print(f"⚠️ Model '{self.model_type}' not found in model_config, fallback to '{first_model_key}'")
            model_instance = self.model_config[first_model_key]  
    ...
    def _objective(self, trial, X, y):
        try:
            ...
            # 如果没有设置 trial_timeout，直接在当前进程运行（兼容以前行为）
            ...
            # 否则使用子进程来运行 cross_val_score 并在超时后终止
            def worker(pipe, X_local, y_local, cv_local, q):
                try:
                    scores_local = cross_val_score(pipe, X_local, y_local, cv=cv_local,
                                                   scoring='accuracy', error_score='raise')
                    q.put(('ok', float(np.mean(scores_local))))
                except Exception as e:
                    q.put(('err', str(e)))

            q = multiprocessing.Queue()
            p = multiprocessing.Process(target=worker, args=(pipeline, X, y, self.cv, q))
            p.start()
            p.join(self.trial_timeout)
            ...
            if p.is_alive():
                ...
                try:
                    ...
                    pass
                print(f"\n⏱️ Trial {trial.number} timed out after {self.trial_timeout} seconds and was terminated.")
                return float('-inf')
            ...
            try:
                ...
                if status == 'ok':
                    ...
                    print(f"\n⚠️ Trial {trial.number} failed inside subprocess: {payload}")
                    return float('-inf')
            except Exception:
                print(f"\n⚠️ Trial {trial.number} ended without result (likely killed or crashed).")
                return float('-inf')
        except Exception as e:
    ...
    def optimize(self, X, y):
        ...
        def early_stop_callback(study, trial):
            nonlocal last_improvement_trial

            # 只有真正刷新 best的 trial 才算 improvement
            if study.best_trial.number == trial.number:
                last_improvement_trial = trial.number

            trials_without_improvement = trial.number - last_improvement_trial

            if trials_without_improvement >= PATIENCE:
                print(f"\n🛑 Early stopping at trial {trial.number}: "
                    f"no improvement for {trials_without_improvement} trials "
        ...
        with tqdm(total=self.n_trials, desc="Optimizing Pipeline") as pbar:
            ...
            try:
                self.study.optimize(
                    ...
                    callbacks=[update_pbar, early_stop_callback]
                )
        ...
        try:
            # 理论上 len(self.study.trials) == 已完成 trial 数
            self.trials_executed = len(self.study.trials) if self.study is not None else 0
            # 也把它放进 user_attrs，便于外部通过 study 访问（冗余，但安全）
            if self.study is not None:
                self.study.set_user_attr("trials_executed", self.trials_executed)
        except Exception:
            # 回退到 n_trials 作为保底
            self.trials_executed = self.n_trials
...
class EnhancedPipelineRunner:
    ...
    新增参数： trial_timeout (秒) —— 用来限制每个 trial 的最大执行时间。
    """
    def __init__(self, file_path, target_column, steps_order_candidates,
                 model_manager: ModelManager, n_trials=50, cv=3, trial_timeout=60):
        self.file_path = file_path
        self.target_column = target_column
        self.steps_order_candidates = steps_order_candidates
        self.n_trials = n_trials
        ...
        self.trial_timeout = trial_timeout  # 新增：每个 trial 的时间上限（秒）
    ...
    def optimize(self):
        ...
        trials_record = {}  # model_key -> trials_executed
        ...
        for model_key in model_keys:
            for steps_order in self.steps_order_candidates:
                ...
                optimizer = BayesianPipelineOptimizer(
                    ...
                    n_trials=self.n_trials,
                    cv=self.cv,
                    trial_timeout=self.trial_timeout  # 把 timeout 传给 optimizer
                )
...
if __name__ == "__main__":
    ...
    # 创建并运行增强运行器（传入 trial_timeout，例如 60 秒）
    runner = EnhancedPipelineRunner(
        ...
        cv=3,
        trial_timeout=60  # 每个 trial 最多允许 60 秒运行（示例）
    )
