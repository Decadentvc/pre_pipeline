...
#            ax.annotate(percentage,                        xy=(rects.get_x() + rects.get_width() / 2, height),                        xytext=(0, 0.5),                        textcoords="offset points",                        ha='center', va='bottom')            
    elif mode == 'count':
        """Display Count"""
        for j in range(len(patches)):
            rects = patches[j]
...
#            ax.annotate(height,                        xy=(rects.get_x() + rects.get_width() / 2, height),                        xytext=(0, 0.5),                        textcoords="offset points",                        ha='center', va='bottom')         
               
#def autoplot(X,hue,data,colors):
#    fig, ax = plt.subplots(1,2,figsize=(15, 10))
#    
#    plt.subplot(1,2,1)
#    ax[0] = sns.barplot(x=X.value_counts().index,                        y=(X.value_counts()/len(X))*100,                        data=data,palette='Blues_d')    
...
df_Train['PaymentMethod'] = df_Train['PaymentMethod'].replace({'Bank transfer (automatic)':'Bank transfer Auto',                                                               'Credit card (automatic)':'Credit card Auto'})
#for i in range(len(IVs)):    
#    autoplot(df_Train[IVs[i]],df_Train['Churn'],df_Train,colors1)
for i in range(len(IVs)):    
    print(pd.crosstab(df_Train[IVs[i]], df_Train['Churn']).apply(lambda r: r/r.sum(), axis=1))
...
#plt.show()
X = X.drop(columns=['OnlineSecurity_No internet service','OnlineBackup_No internet service',                    'DeviceProtection_No internet service','TechSupport_No internet service',                    'StreamingMovies_No internet service','StreamingTV_No internet service'])
...
import numpy as np
np.save("haipipe/core/tmpdata/prenotebook_res/janelsmb_telco-customer-churn.npy", { "accuracy_score": score })
