...
no_m_v = pd.DataFrame(no_m_v).reset_index(drop=True).infer_objects()
cat_train_x, num_train_x = catch_num(no_m_v)
...
no_m_v = pd.concat([cat_train_x.reset_index(drop=True), num_train_x.reset_index(drop=True)],axis=1)
...
add_seletcion.fit(no_m_v)

cols = list(no_m_v.columns)
mask = add_seletcion.get_support(indices=False)
final_cols = list(compress(cols, mask))
no_m_v = pd.DataFrame(add_seletcion.transform(no_m_v), columns=final_cols)
...
import numpy as np
#np.save("haipipe/core/tmpdata/merge_max_result_rl/kulchorobeknazarov_heart-disease-prediction/133.npy", { "accuracy_score": score })
np.save("haipipe/core/tmpdata/rl_cross_val_res/kulchorobeknazarov_heart-disease-prediction/133.npy", { "accuracy_score": cross_score })
