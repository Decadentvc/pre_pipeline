...
data_y = pd.DataFrame(data_y).reset_index(drop=True).infer_objects()
add_engine = PolynomialFeatures(interaction_only=True, include_bias=False)
add_engine.fit(data_y)
train_data_x = add_engine.transform(data_y)
train_data_x = pd.DataFrame(train_data_x)
data_y = train_data_x.loc[:, ~train_data_x.columns.duplicated()]
...
add_seletcion.fit(data_y)

cols = list(data_y.columns)
mask = add_seletcion.get_support(indices=False)
final_cols = list(compress(cols, mask))
data_y = pd.DataFrame(add_seletcion.transform(data_y), columns=final_cols)
...
import numpy as np
#np.save("haipipe/core/tmpdata/merge_max_result_rl/datascientist25_gender-recognition-by-voice-using-machine-learning/11.npy", { "accuracy_score": score })
np.save("haipipe/core/tmpdata/rl_cross_val_res/datascientist25_gender-recognition-by-voice-using-machine-learning/11.npy", { "accuracy_score": cross_score })
